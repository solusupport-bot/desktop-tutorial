...
def travelpayouts_drive_script(cfg):
    """
    Travelpayouts "Drive" auto-inserts affiliate links/widgets where visitors
    click most. Blank until automation/config.json's travelpayouts_drive_src
    is set to the script URL from the Travelpayouts Drive dashboard.
    """
    src = cfg.get("travelpayouts_drive_src", "").strip()
    if not src:
        return ""
    return (
...
{goatcounter_script(cfg)}{travelpayouts_drive_script(cfg)}</head>
<body>
