...
## 🎨 sepia로 콘텐츠 탈AI 처리 (2026-09-07 설치)

**sepia는 AI 글의 구조적 특징을 제거하는 전문 도구**입니다. 단순 문법 수정이 아닌, 서사 구조/담론 흐름/문체를 깊이 있게 개선합니다.

### 설치 상태
- Claude Code에 `/sepia` 플러그인 설치 완료 (사용자 스코프)
- 마켓플레이스 추가: Nanako0129/sepia

### 적용 대상
SNS 게시물, 블로그 글, 기술 문서 등 **모든 콘텐츠 작성 시**:
1. **Agent Reach로 실제 데이터 조사** (위의 SNS 콘텐츠 규칙 참고)
2. **초안 작성**
3. **sepia로 탈AI 처리** (아래 선택지 중 적절한 것 사용)

### 사용 가능한 커맨드

| 커맨드 | 용도 | 추천 상황 |
|--------|------|---------|
| `/sepia-review` | 진단만 (수정X) | 문제점만 파악하고 싶을 때 |
| `/sepia-refactor` | 최소 편집 | 구조 유지하고 부분 개선 원할 때 |
| `/sepia-recreate` | 완전 다시 쓰기 | 처음부터 깔끔하게 다시 쓸 때 |

### 규칙 (필수)

- **픽션 vs 전문 문서는 구분**: 소설과 기술 문서의 규칙이 다름
- **과도하게 적용하지 말 것**: sepia는 3–5개의 선택적 개선만 적용하도록 설계됨 (모든 규칙을 다 적용하면 새로운 AI 지문이 됨)
- **언어별 최적화**: 영문이 주이나, 중문(간체/번체) 지원함 (한글은 기본 영문 규칙 적용)
- **사실 근거 유지**: sepia도 "지어내기" 하지 않음 — 진짜 조사한 내용만 개선할 것

### 근거 (학술)

sepia는 2026년 최신 연구 13개 이상을 근거로 함 (StoryScope, LAMP, CHI/ACL/EMNLP 등).
자세한 내용은 https://github.com/Nanako0129/sepia/tree/main/research 참고.
