 ...
 * curate.js의 Threads 포맷은 "훅 -> 관찰 1~2개 -> 주의사항 -> 질문"을 빈 줄로
 * 구분한 문단들로 만들어지므로, 첫 \n\n에서 자르는 것만으로 자연스럽게 나뉩니다.
 * 문단이 하나뿐이면(구분자가 없으면) 나눌 수 없으니 훅=전체, reply=null.
 */
const splitHookAndReply = (text) => {
  const idx = text.indexOf('\n\n');
  if (idx === -1) return { hook: text, reply: null };
  return { hook: text.slice(0, idx).trim(), reply: text.slice(idx + 2).trim() };
};
  ...
  try {
    ...
    if (reply) {
      try {
        ...
        const message = apiError ? `${apiError.message} (code=${apiError.code}, subcode=${apiError.error_subcode})` : err.message;
        log.warn(`Threads 답글(2페이지) 발행 실패(원 게시물은 정상): ${message}`);
      }
    }
