...
const publishToThreads = async ({ text, imageUrl, imageUrls, videoUrl }) => {
  ...
  try {
    ...
    // 2026-09-13/14 실측: waitForThreadsContainer가 status===FINISHED를 확인한
    // 직후에도 threads_publish를 바로 호출하면 "The requested resource does not
    // exist (code=24, subcode=4279009)"가 난다 — 답글 체인에는 이미 이 레이스에
    // 대한 재시도가 있었는데(아래), 정작 원 게시물 publish에는 없어서 캐로셀/이미지/
    // 영상 게시물이 컨테이너 생성까지는 성공하고도 최종 발행 단계에서 반복적으로
    // 실패하고 있었다. 답글과 동일한 패턴(짧은 간격 재시도)을 여기에도 적용한다.
    let publishRes;
    let publishErr;
    for (const delayMs of [0, 3000, 5000]) {
    ...
    //
    // 참여 속도(velocity) 시딩(2026-09-02, 리서치 근거: Threads 알고리즘은 초반 참여
    // 속도를 도달의 1순위 요인으로 취급하고, 답글 전체가 30초 안에 몰아서 올라오면
    // 실시간 대화처럼 보이지 않아 팔로워가 다시 들어와 볼 이유가 없다) — 첫 답글
    // 직후 빠른 신호 → 이후 점진적 간격으로 알림/피드 반복 노출(2026-09-06 최적화:
    // 초반 engagement velocity 강화 — 30초, 2분, 5분, 7분 간격으로 조정).
    const REPLY_SPACING_SEQUENCE = [30 * 1000, 2 * 60 * 1000, 5 * 60 * 1000, 7 * 60 * 1000];
    let previousId = publishRes.data.id;
    for (let i = 0; i < replyParts.length; i += 1) {
      if (i > 0) await wait(REPLY_SPACING_SEQUENCE[Math.min(i - 1, REPLY_SPACING_SEQUENCE.length - 1)]);
      let lastErr;
      let replyId;
