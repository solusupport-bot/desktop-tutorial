...
const publishToThreads = async ({ text, imageUrl, imageUrls, videoUrl }) => {
  ...
  try {
    ...
    // 실시간 대화처럼 보이지 않아 팔로워가 다시 들어와 볼 이유가 없다) — 두 번째
    // 답글부터는 몰아서 발행하지 않고 몇 분씩 텀을 둬, 팔로워 알림/피드에 여러 번
    // 걸쳐 노출되도록 한다. 첫 번째 답글은 레이스 컨디션 회피용 3초 대기만 유지.
    const REPLY_SPACING_MS = 3 * 60 * 1000;
    let previousId = publishRes.data.id;
    for (let i = 0; i < replyParts.length; i += 1) {
      if (i > 0) await wait(REPLY_SPACING_MS);
      let lastErr;
      let replyId;
