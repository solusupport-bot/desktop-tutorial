...
/**
 * 2026-09-17 실측 버그: data/topic_blog_links.json에 주제 31개 전부의 slug가 이미
 * 채워져 있었는데, 그중 6개는 실제로 content/posts/에 파일이 없는 "유령 링크"였다
 * (원인 불명 — 아마 파일 쓰기 실패 없이 슬러그만 먼저 채워진 과거 실행이나 수동 편집).
 * sync-blog-posts.js의 "이미 링크가 있으면 스킵" 로직이 이 6개를 "이미 작성됨"으로
 * 착각해 매번 건너뛰면서, 결과적으로 2026-09-11 이후 새 블로그 글이 전혀 안 써졌다
 * (topic_blog_links.json이 사실상 모든 주제를 "완료"로 채워버려 pending이 항상 0이었음).
 * 링크 맵의 키 존재 여부만 믿지 말고, 실제 파일이 있는지 GitHub API로 직접 확인한다.
 */
const listExistingSlugs = async () => {
  const pat = process.env.BLOG_REPO_PAT;
  if (!pat) return null;
  try {
    const { data } = await axios.get(
      `https://api.github.com/repos/${BLOG_REPO}/contents/content/posts`,
      { headers: { Authorization: `Bearer ${pat}`, Accept: 'application/vnd.github+json' } }
    );
    return new Set(data.filter((f) => f.name.endsWith('.md')).map((f) => f.name.replace(/\.md$/, '')));
  } catch (err) {
    log.warn(`블로그 저장소 글 목록 조회 실패, 링크 맵만 신뢰합니다: ${err.response?.data?.message || err.message}`);
    return null;
  }
};
...
module.exports = { pushBlogPost, listExistingSlugs, BLOG_BASE_URL };
