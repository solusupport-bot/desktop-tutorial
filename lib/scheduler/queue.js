...
const addPost = ({ text, imageUrl, imageUrls, videoUrl, platforms, scheduledAt, ...extra }) => {
  ...
  const item = {
    ...
    // 2026-09-18 사용자 결정: 사람 검수 없이 항상 자동 발행한다 — 기본값을 true로
    // 바꿔 run.js의 approved 하드 게이트를 사실상 통과시킨다. verifyStatus(자동 검증)
    // 게이트는 그대로 유지 — 중복 이미지/주제-이미지 불일치 같은 기계적 오류는 계속 막는다.
    approved: true,
