 ...
 * "소유권" 안전장치가 그대로 적용된다.
 *
 * 2026-09-16: 대시보드(dashboard/)의 검수(approved)/검증(verifyStatus) 하드 게이트를
 * 'pending' 경로에만 추가한다 — 이 함수가 status를 'claimed'로 바꾸는 유일한 지점이라
 * (grep으로 확인) 여기 두 줄이 daily-auto-post.js 등 모든 호출자에 적용된다. 이미
 * 한 번 승인·검증을 통과해 'claimed'까지 갔다가 'partial'이 된 항목은 재시도 시
 * 다시 검사하지 않는다 — approved/verifyStatus는 최초 승인 이후 바뀌지 않기 때문.
 */
const claimDuePosts = () => {
  ...
  for (const item of queue) {
    if (item.status === 'pending') {
      ...
      if (!item.approved) continue; // 검수 하드 게이트
      if (item.verifyStatus === 'failed') continue; // 검증 하드 게이트
      item.status = 'claimed';
      changed = true;
