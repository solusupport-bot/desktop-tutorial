...
const runDuePosts = async () => {
  ...
  for (const item of queue) {
    ...
    for (const platform of item.platforms) {
      ...
      if (handler.requiresMedia && !item.imageUrl && !(item.imageUrls && item.imageUrls.length)) {
        log.err(`${platform}은 이미지가 필요한데 imageUrl이 없습니다. 건너뜁니다.`);
        item.results[platform] = { error: 'imageUrl required' };
        allOk = false;
        continue;
      }

      const result = await handler.publish({ text: item.text, imageUrl: item.imageUrl, imageUrls: item.imageUrls });
