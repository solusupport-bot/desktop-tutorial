...
const TOPIC_QUERIES = {
  "Hailing a taxi in Korea: Kakao T vs. street taxis vs. airport 'deluxe' cabs": [
    "hand holding phone taxi app Seoul",
    "orange Seoul taxi on street korea"
  ],
  "Why Google Maps fails in Korea and how the road-name address system actually works": [
    "phone map app street Seoul korea",
    "korean road name address sign building"
  ],
  "Seoul subway details nobody tells you: exit numbers, transfer walks, express trains, and the last train": [
    "Seoul subway exit number sign stairs",
    "Seoul subway platform train arriving korea"
  ],
  "Short-term vs. long-term housing in Korea: guesthouse, goshiwon, officetel, and the deposit system": [
    "tiny single room korean goshiwon interior",
    "Seoul officetel apartment buildings korea skyline"
  ],
  "Opening a Korean bank account as a foreigner — and why your ARC and phone number come first": [
    "korean bank branch counter teller seoul",
    "mobile banking payment app phone korea"
  ],
  ...
  // scripts/verify-topic-media-alignment.js가 2026-09-02 실측으로 잡아낸 누락 —
  // 이 주제는 추가된 이후로 계속 이미지 검색어가 없어 이미지 없이(또는 고정
  // 대표 이미지로만) 발행되고 있었다.
  'KTX vs. SRT vs. intercity bus for long-distance travel': [
    'ktx srt high speed train seoul station korea', // angle0: KTX/SRT 역 비교
    'korea intercity express bus terminal' // angle1: 시외/고속버스
  ],
  // 2026-09-11 추가 8개 주제(사용자 요청: "매일 블로그 글이 중복없이 올라가도록") —
  // 위와 동일한 규칙: 배열 순서가 korea_travel.js 해당 주제 content 배열과 1:1 대응.
  'Insadong traditional culture street': [
    'insadong seoul korea traditional tea house alley', // angle0: 전통찻집/한지공예
    'insadong seoul korea pedestrian street craft shops' // angle1: 일요일 차없는거리/쌈지길
  ],
  'Everland vs. Lotte World theme parks': [
    'everland theme park korea rides', // angle0: 에버랜드 야외 놀이공원
    'lotte world seoul korea indoor amusement park' // angle1: 롯데월드 지하철 접근성
  ],
  'Hongdae nightlife and youth culture district': [
    'hongdae seoul korea street performance night', // angle0: 홍대 버스킹/놀이터
    'hongdae seoul korea nightlife bars clubs street' // angle1: 클럽/바 거리
  ],
  'Nami Island and Petite France day trip': [
    'nami island korea tree lined path ferry', // angle0: 남이섬 나무길/페리
    'petite france gapyeong korea colorful village' // angle1: 프티프랑스
  ],
  'Suwon Hwaseong Fortress': [
    'suwon hwaseong fortress korea wall walk', // angle0: 성곽 걷기
    'suwon hwaseong haenggung palace korea' // angle1: 화성행궁
  ],
  'Domestic flights vs. KTX for Jeju and long-distance trips': [
    'gimpo airport korea domestic terminal', // angle0: 김포-제주 국내선
    'ktx train platform seoul station korea' // angle1: KTX vs 항공 비교
  ],
  'Seoul attraction pass comparison': [
    'seoul palace ticket entrance korea tourist', // angle0: 패스 활용 계산
    'seoul tourist attraction pass scan korea' // angle1: 패스 스캔/활성화
  ],
  'Korean public holidays that disrupt travel plans': [
    'seoul highway traffic holiday korea', // angle0: 추석/설날 교통 정체
    'korean traditional market closed holiday' // angle1: 명절 휴업 상점
  ],
  // scripts/verify-topic-media-alignment.js가 2026-09-11 재점검에서 발견한 기존 누락 —
  // 이 4개 주제는 추가된 이후로 계속 이미지 검색어가 없어 이미지 없이(또는 Odii
  // 폴백에만 의존해) 발행되고 있었다.
  'Namdaemun Market': [
    'namdaemun market seoul korea traditional stalls', // angle0: 남대문시장 도매 상가
    'namdaemun market seoul korea street food stalls' // angle1: 길거리 음식/면세
  ],
  'Trash and recycling rules for residents': [
    'korea apartment trash bag official designated', // angle0: 종량제 봉투
    'korea recycling sorting bins apartment building' // angle1: 재활용 분리수거
  ],
  'DMZ and JSA tours': [
    'dmz korea observatory tour third tunnel', // angle0: DMZ 관측소/제3땅굴
    'jsa panmunjom korea joint security area' // angle1: JSA 판문점
  ],
  'Jjimjilbang etiquette': [
    'jjimjilbang korea sauna lounge common area', // angle0: 공용 라운지/사우나
    'jjimjilbang korea locker room check-in counter' // angle1: 체크인/로커
  ]
};
...
const findKoreaPhoto = async (apiKey, rawQuery, recentUrls = []) => {
  ...
  try {
    ...
    // 2026-09-13 사용자 지적: "다른 플랫폼 이미지는 한국이 아닌 다른 사진도 올라온다"
    // — 예전엔 여기서 alt에 한국 신호가 없어도 "관련성만으로" 채택하는 완화된 재시도가
    // 있었는데, 이게 바로 외국 사진이 새어 들어오는 통로였다. 이 파일 상단 주석과
    // 함수 설명 자체가 "한국임이 확인 안 되면 이미지 없이 발행"이라고 명시하고 있었는데
    // 코드는 정반대로 동작하고 있었다 — 완화 재시도를 완전히 제거하고, 한국 신호가
    // 확인된 사진만 채택한다. 못 찾으면 null을 반환해 호출부(resolveImages)가 Pixabay ->
    // 고정 대표 이미지 순으로 폴백하게 한다.
  } catch (err) {
