 ...
 * 뒤로 밀려 신호가 약했다. "south korea"를 검색어 맨 앞으로 옮기고, 영상엔 alt가 없는
 * 대신 Pexels 자동 생성 슬러그(video.url)에 KOREA_SIGNAL이 있는 것만 채택한다
 * (2026-09-13: 슬러그에 신호가 없을 때 "관련성만으로" 채택하던 완화 재시도는 완전히
 * 제거했다 — 그게 바로 한국 아닌 나라 영상이 새어 들어오던 통로였다).
 *
 * 주제당 영상 검색어가 1페이지(15개)만 뒤지던 예전엔 후보 풀이 금방 소진돼 반복
 * 노출로 체감됐다 — 이미지처럼 여러 페이지(최대 8페이지 = 120개 후보)를 뒤지도록
 * 넓힌다. 완화 재시도 제거로 실제 채택 가능한 후보가 줄어든 만큼, 페이지 수를
 * 이미지와 동일한 8로 맞춰 보완한다.
 */
const MAX_PAGES = 8;
  ...
  try {
    ...
    for (let page = 1; page <= MAX_PAGES; page += 1) {
      const res = await axios.get(PEXELS_VIDEO_SEARCH_URL, {
        ...
        params: { query, per_page: 15, page, orientation: 'portrait' },
        timeout: 15000
      });
      const videos = res.data?.videos || [];
      if (!videos.length) break;
      pagesOfVideos.push(videos);
    }
    const allVideos = pagesOfVideos.flat();
    if (!allVideos.length) {
    ...
    // — 영상도 이미지와 같은 구멍이 있었다: 슬러그에 한국 신호가 없으면 "관련성만으로"
    // 아무 영상이나 채택하는 완화된 재시도가 있어, 실제로는 한국이 아닌 나라 영상이
    // 새어 들어올 수 있었다. 완전히 제거 — 한국 신호가 확인된 영상만 채택하고,
    // 없으면 null을 반환해 호출부가 Pixabay -> 이미지 캐로셀 순으로 폴백하게 한다.
    log.warn(`"${query}" 검색 결과 중 한국 확인 + 미사용 조건을 만족하는 영상이 없습니다.`);
    return null;
  } catch (err) {
