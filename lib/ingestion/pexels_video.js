 ...
 *
 * 같은 날 사용자가 "자꾸 중복된 사진이 올라온다"고 재차 지적한 근본 원인도 여기 있었다:
 * 주제당 영상 검색어가 딱 1개 고정이고, 그 검색어로 딱 1페이지(15개)만 뒤졌다 —
 * 후보 풀 자체가 15개뿐이니 며칠만 지나도 다 소진돼 반복될 수밖에 없었다. 이미지처럼
 * 여러 페이지(최대 5페이지 = 75개 후보)를 뒤지도록 넓힌다.
 */
const MAX_PAGES = 5;
