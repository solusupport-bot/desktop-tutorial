...
// data.go.kr의 일반 인증키(Encoding)가 Secret에 저장된 경우,
// axios가 query parameter를 다시 인코딩하기 전에 한 번만 decode한다.
// Decoding 키가 저장된 경우에는 그대로 사용한다.
...
const request = async ({ service, regionName, numOfRows = 20, pageNo = 1 }) => {
  ...
  const key = normalizeServiceKey(process.env[config.keyEnv]);
...
module.exports = { fetchRegionSamples, request, REGIONS, normalizeServiceKey };
