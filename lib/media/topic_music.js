...
// 2026-09-16 실측(Openverse API 직접 호출, license_type=commercial,modification):
// 위에 쓰던 "traditional korean"/"traditional ambient"/"cinematic ambient"/
// "upbeat market"/"korean pop" 같은 2~3단어 조합은 전부 0건이었다 — 태그 기반
// 검색이라 정확히 그 조합이 붙은 곡이 카탈로그에 없으면 조용히 실패하고 다음
// 후보로 넘어간다(결과적으로 "명동"류 6개 주제만 빼고 나머지는 사실상 매번
// 범용 ambient/pop으로 흘러가고 있었음). 반면 단일 단어는 대부분 결과가 있다
// (korean:5, traditional:74, cinematic:240, pop:115, market:14, beach:101,
// acoustic:91, electronic:206). 그래서 각 주제를 "주제 관련 단일 단어 →
// pop/upbeat 단일 단어 → 범용 폴백" 순서로 재구성한다 — 실제로 결과가 나오는
// 검색어를 먼저 쓰게 하면서, 동시에 Jamendo 우선 선택(openverse_music.js)과
// 합쳐 "주제에 맞으면서도 노래 같은" 곡이 먼저 걸릴 확률을 높인다.
const ATTRACTION_MOODS = {
  '경복궁': ['korean', 'traditional', 'pop', 'ambient'],
  '북촌한옥마을': ['korean', 'traditional', 'pop', 'ambient'],
  'N서울타워': ['cinematic', 'pop'],
  '명동': ['pop', 'upbeat'],
  '해운대': ['beach', 'pop', 'chill'],
  '제주도': ['acoustic', 'pop', 'chill'],
  '남대문시장': ['market', 'pop', 'upbeat'],
  '인사동': ['korean', 'traditional', 'pop', 'ambient'],
  '에버랜드': ['playful', 'pop', 'upbeat'],
  '홍대': ['electronic', 'pop', 'upbeat'],
  '남이섬': ['acoustic', 'pop', 'chill'],
  '수원화성': ['korean', 'traditional', 'pop', 'ambient']
};

const ATTRACTION_FALLBACK = ['pop', 'cinematic', 'ambient'];
const TIP_MOOD = ['pop', 'upbeat', 'electronic'];
