...
const getThreadsStatLine = () => {
  ...
  if (measured) {
    return `Our own ${measured.category_type} Threads posts here average ${measured.avg_views} views.`;
  }
  const optimalTime = threadsPerf.metadata && threadsPerf.metadata.optimal_posting_time;
  return optimalTime ? `Threads posts here get the most early traction around ${optimalTime}.` : null;
};
...
const PLATFORM_GUIDE = {
  threads: 'Purpose: turn a real first-timer friction point into a short reply-chain thread — the account '
    ...
    + 'One observation paragraph should cite our own measured Threads performance from '
    + 'data/platform_performance.json when a category has avg_views > 0; if every category is still 0 '
    + '(not yet measured), cite the documented optimal_posting_time instead — never invent a view count '
    + 'or engagement rate that is not in that file. '
