...
const platformPerformance = require('../../data/platform_performance.json');

// 2026-09-12: "구체적 통계" 요청 — data/platform_performance.json은 아직 실측치가
// 전부 0(측정 안 됨)이라, 여기서 숫자를 지어내면 이 저장소 CLAUDE.md의 "조회수를
// 지어내지 말 것" 규칙을 정면으로 어기게 된다. 그래서 avg_views > 0인 실측 카테고리가
// 생기면 그걸 그대로 인용하고, 아직 없으면 이미 문서화된(지어낸 게 아닌) optimal
// posting time으로 대체한다. weekly-performance-review.js가 실측치를 채우기
// 시작하면 자동으로 실제 숫자가 캡션에 반영된다.
const getThreadsStatLine = () => {
  const threadsPerf = platformPerformance.threads;
  if (!threadsPerf) return null;
  const measured = Object.values(threadsPerf.by_category || {})
    .filter((c) => c.avg_views > 0)
    .sort((a, b) => b.avg_views - a.avg_views)[0];
  // 2026-09-17: optimal_posting_time("우리가 몇 시에 올리는지")을 캡션에 넣던 폴백을
  // 없앴다 — 실측 조회수(avg_views)는 독자에게 "이 계정 팔로우할 만하다"는 사회적
  // 증거로 의미가 있지만, "우리 계정은 몇 시에 올린다"는 여행자에게 아무 쓸모가
  // 없는 운영 정보다. 실제로 quality_gate.js가 이 문장이 낀 캡션을 "주제와 무관한
  // 내부 운영 메모"로 반려하며 발견했다. 측정치가 없으면 그냥 통계 얘기를 안 한다
  // (호출부가 null을 받으면 일반 관찰 문장으로 대체).
  return measured ? `Our own ${measured.category_type} Threads posts here average ${measured.avg_views} views.` : null;
};
...
const PLATFORM_GUIDE = {
  threads: 'Purpose: turn a real first-timer friction point into a short reply-chain thread — the account '
    ...
    + 'One observation paragraph: {{THREADS_STAT_LINE}} '
  ...
  bluesky: 'Purpose: same reply-chain structure as threads, but for an audience that skews toward people who '
    ...
    + 'naturally. Hashtag strategy: prioritize established discovery (#TravelBlog, #KoreaTravel) + 1 niche tag '
    + '(#SeoulTrip, #FirstTimeInKorea). Do not add a link — the publisher appends the blog URL itself.',
  mastodon: 'Purpose: same reply-chain structure as threads, for a fediverse audience that rewards genuinely '
    ...
    + 'Keep each paragraph under 450 characters. 3-5 hashtags maximum — on Mastodon hashtag timelines '
    + 'federate across instances and are a real discovery path. Strategy: 2-3 primary tags (#KoreaTravel, '
    + '#TravelTips, #Seoul) + 1-2 emergent trends (monitor news for #Korea, #SafetyTips relevance). '
    + 'Place hashtags in final reply for federation preference. Do not add a link — the publisher appends the blog URL.',
  reddit: 'Purpose: seed meaningful discussion in relevant subreddits while avoiding "promotional" flagging. '
    + 'Format: lead with context statement that shows genuine value to the subreddit (not a generic hook) '
    + '-> 2-3 practical points with specific details (won amounts, place names, time estimates) '
    + '-> plant blog link naturally in replies (not in post body — subreddits penalize promotional links in posts). '
    + 'Subreddit strategy: r/korea (primary) + r/travel or r/IWantOut (secondary based on content type). '
    + 'Write like you\'re answering a specific question someone in that community would have, not like you\'re '
    + 'promoting anything. Reddit karma comes from genuine helpfulness, not marketing.',
  pinterest: 'Purpose: drive click-throughs to blog via search engine optimization and visual discovery. '
    + 'Format: keyword-dense description with 2-3 concrete tips (specific won amounts, place names, '
    + 'time estimates, day counts) in numbered format -> blog link and call-to-action. '
    + 'SEO strategy: first line = topic + benefit (e.g. "Korea eSIM costs less than roaming: here\'s how"). '
    + 'Rich Pins preferred (description + image metadata). Alt text required for accessibility + discovery. '
    + 'Board strategy: search 50k+ follower boards in Travel/Korea category, avoid niche boards under 10k followers. '
    + 'Create 2-3 visual variations per blog post (different title crops, accent colors).',
  tumblr: 'Purpose: a single self-contained post (no reply chain, length is not a constraint) for an '
    ...
    + 'separately (10-20 high-volume tags: #koreatravel, #travelblogs, #southkorea, #tourismkorea, etc. '
    + 'plus 5 niche tags matching content specifics). Tag strategy: reach volume tags first, niche second. '
    + 'Do not add a link — the publisher appends the blog URL.'
};
...
// Phase 2.1 (2026-09-06): Threads question expansion
// Benchmarked against high-performing accounts — top 3 question types:
// 1. Loss/experience-sharing ("Anyone else lose money...") = social proof + cost concern
// 2. Crowdsourced advice ("What would you add...") = engagement from community
// 3. Direct CTA + experience-sharing ("Worth knowing...") = low-friction explicit action
// Rotate by seed to ensure variety across same topics (prevent repetition when same topic repeats).
const THREADS_QUESTIONS = [
  ...
  'What surprised you the most when you landed?'
];
...
// Phase 2.1 (2026-09-06): 모든 Facebook CTA를 질문형으로 통일 — 엔게이지먼트 벤치마킹에서
// 진정한 질문 마무리가 댓글/답글 유도력이 높다는 걸 확인했기 때문.
const FACEBOOK_CTAS = [
  ...
  'What\'s your biggest tip for first-timers landing in Korea?',
  'Have you learned this the hard way? Tell your story.',
...
// 2026-09-17 사용자 지적(조회수/팔로우 저조, 최신 조사 결과 "정형화된 AI 콘텐츠는
// 알고리즘이 점점 밀어낸다"): 6개뿐이던 오프닝을 늘려 같은 문장이 6게시물마다
// 반복되는 걸 줄인다 — 근본적으로는 훅-브릿지-리졸브-CTA라는 4박자 구조 자체가
// 매번 똑같은 게 더 큰 문제라 이것만으로 다 해결되진 않지만, 캐시/템플릿 폴백
// 경로(ANTHROPIC_API_KEY 없을 때)에서 바로 체감되는 반복을 줄이는 안전한 선에서만 손댄다.
const PROMISE_OPENERS = [
  ...
  'Ask five people and you\'ll get five different answers on this.',
  'Here\'s what actually happens once you\'re standing there.',
  'The guidebooks skip this part entirely.',
  'You won\'t find this in the usual travel advice.',
  'Locals do this differently than every blog post says.',
  'This is the part that separates a smooth trip from a stressful one.'
];
...
const reshapeByTemplate = (rawItem, platform, seed = 0) => {
  ...
  if (platform === 'threads') {
    ...
    // 약속(궁금증 유발) -> 반전(구체적 훅) -> 관찰 최대 2개(실측/문서화된 통계 1개 포함) ->
    // 질문 = 최대 5조각(MAX_THREAD_PARTS=5 유지). 통계 문장을 최우선으로 넣고 남은
    // 자리에 원문 관찰을 채운다 — 문장이 부족하면 자연스럽게 조각 수가 줄어든다.
    const statLine = getThreadsStatLine();
    const observations = [statLine, ...rest].filter(Boolean).slice(0, 2);
...
const curateContent = async (rawItem, platforms = ['threads', 'facebook', 'instagram'], seed = 0) => {
  // 2026-09-17 실측 버그: seed는 pickNextTopic()이 "지금까지 쓴 전체 게시물 수"
  // (state.history.length)로 매길 뿐 어떤 주제인지와 무관하다 — 반면 이 캐시는
  // "seed 37일 때 쓸 주제는 X일 것"이라는 예상하에 미리 써둔 것이라, topic_bank.json에
  // 새 주제가 추가되거나 순서가 바뀌면(rankByLeastRecentlyUsed) 같은 seed가 가리키는
  // 실제 주제가 달라진다. 그 결과 캐시된 캡션이 완전히 다른 주제로 발행되는 사고가
  // 실제로 발생했다(3배치 21건, "Useful travel apps"에 경복궁 캡션이 붙는 식) — 큐레이션
  // 자체는 멀쩡한데 검수 승인이 막혀 있던 덕에 발행 전에 발견해 반려할 수 있었다.
  // 캐시 항목마다 원래 어떤 주제용이었는지(source)가 이미 저장돼 있으므로, seed가
  // 같아도 주제가 다르면(rawItem.source와 불일치) 캐시를 쓰지 않고 Claude/템플릿으로
  // 폴백한다 — 이러면 seed 재사용 문제 자체가 안전하게 무력화된다.
  const cached = snsContentQueue.get(seed);
  const queued = cached && cached.source === rawItem.source ? cached : null;
  if (cached && !queued) {
    log.warn(`캐시된 콘텐츠(seed=${seed})의 주제("${cached.source}")가 실제 주제("${rawItem.source}")와 달라 사용하지 않습니다.`);
  }
  ...
  // 2026-09-17 실측 버그(quality_gate.js가 최초로 잡아냄): threads 가이드가 Claude에게
  // "0이면 optimal_posting_time을 인용하라"는 조건 로직을 그대로 던져주니, 실제 숫자
  // 대신 "아직 측정 안 됐다"는 걸 설명하는 메타 문장("Our own Threads views aren't
  // measured yet...")을 캡션에 그대로 써버렸다 — 발행하면 이상해 보일 뻔한 걸 검증기가
  // 막았다. getThreadsStatLine()이 이미 만들어주는 완성된 문장(또는 통계가 전혀 없을 때
  // 뭘 해야 하는지)을 직접 프롬프트에 박아 넣어 Claude가 조건을 스스로 추론하지 않게 한다.
  const threadsStatLine = getThreadsStatLine();
  const threadsStatInstruction = threadsStatLine
    ? `naturally reword this fact into the paragraph, don't quote it verbatim: "${threadsStatLine}"`
    : 'a genuinely useful practical tip — do not mention analytics, view counts, measurement status, or posting times at all (no performance data exists yet)';
  const guide = platforms
    .map((p) => `- ${p}: ${(PLATFORM_GUIDE[p] || 'Write naturally.').replace('{{THREADS_STAT_LINE}}', threadsStatInstruction)}`)
    .join('\n');
...
[Structure — hook, bridge, resolve, CTA]
Every draft should move through these four beats, adapted to each platform's guide below:
1. Hook (first line): name the reader's confusion or mistake specifically — not a generic teaser.
2. Bridge: one sentence connecting that hook to why the rest is worth reading.
3. Resolve: the actual useful fact(s) from the source material — concrete, specific, no invented numbers.
4. CTA: what the reader does next (per the platform guide — comment, save, read more, etc.), never a bare "learn more".
Write like a person sharing something they just learned, not an ad. Vary sentence length and phrasing across platforms — don't repeat the same clause structure in every draft.
